                ...
                <div className="p-5 space-y-5">
                  <div className="space-y-2">
                    ...
                    {referenceImg ? (
                      <div className="relative rounded-lg overflow-hidden border border-border/70">
                        ...
                        <Badge className="absolute bottom-1.5 left-1.5 bg-black/60 text-white border-0 text-[10px] backdrop-blur">Image-to-image</Badge>
                      </div>
                    ) : (
                      <button onClick={() => fileRef.current?.click()} className="w-full h-20 rounded-lg border border-dashed border-border/70 bg-background hover:bg-secondary/40 hover:border-[hsl(var(--primary))]/50 transition-colors flex flex-col items-center justify-center gap-1 text-muted-foreground hover:text-foreground">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => setPrompt(PROMPT_IDEAS[Math.floor(Math.random() * PROMPT_IDEAS.length)])} className="text-[11px] text-[hsl(var(--primary-deep))] hover:underline inline-flex items-center gap-1">
                        <Sparkles className="h-3 w-3" /> Inspire me
                      </button>
                    </div>
                    <Textarea id="prompt" rows={4} value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="A pearl necklace on marble…" className="resize-none rounded-lg border-border/70 bg-background text-sm focus-visible:ring-[hsl(var(--primary))]" />
                    <Button type="button" onClick={enhancePrompt} disabled={enhancing || !prompt.trim()} variant="outline" size="sm" className="w-full rounded-lg h-8 text-xs">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="neg" value={negative} onChange={(e) => setNegative(e.target.value)} placeholder="blurry, low quality, watermark" className="h-9 rounded-lg border-border/70 bg-background text-sm" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {STYLES.map((s) => (
                        <button key={s.id} onClick={() => setStyle(s.id)} className={`px-3 py-1.5 rounded-full text-xs border transition-colors ${style === s.id ? "bg-[hsl(var(--primary))] text-primary-foreground border-[hsl(var(--primary))]" : "bg-background border-border/70 text-muted-foreground hover:text-foreground hover:border-border"}`}>
                          {s.label}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-4 gap-1.5">
                      {RATIOS.map((r) => (
                        <button key={r.id} onClick={() => setRatio(r.id)} className={`flex flex-col items-center gap-1 py-2.5 rounded-lg border text-[11px] transition-colors ${ratio === r.id ? "bg-secondary border-[hsl(var(--primary))]/40 text-foreground" : "bg-background border-border/70 text-muted-foreground hover:text-foreground"}`}>
                          ...
                          {loading || !item ? (
                            ...
                            <>
                              ...
                              <div className="absolute top-2.5 right-2.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => toggleSave(item)} className={`h-7 w-7 rounded-md backdrop-blur flex items-center justify-center transition-colors ${isSaved(item) ? "bg-[hsl(var(--primary))] text-primary-foreground" : "bg-black/55 text-white hover:bg-black/70"}`} title={isSaved(item) ? "Saved" : "Save to collection"}>
                                  {isSaved(item) ? <Check className="h-3.5 w-3.5" /> : <Heart className="h-3.5 w-3.5" />}
                                </button>
                                <button onClick={() => { navigator.clipboard.writeText(prompt); toast.success("Prompt copied"); }} className="h-7 w-7 rounded-md bg-black/55 text-white backdrop-blur flex items-center justify-center hover:bg-black/70" title="Copy prompt">
                              ...
                              <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/85 via-black/40 to-transparent text-white opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <div className="flex gap-1.5">
                                  ...
                                  <Button size="sm" variant="secondary" className="rounded-md h-7 px-2 text-xs" onClick={() => sendToEdit(item.src)} title="Edit">
